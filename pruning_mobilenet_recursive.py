...
# prune: 0.05 ~ 0.9
# finetune: 0 ~ 200
model_path = f"{model_name}/{layer}/recursive"
...
def test(model, epoch, iteration=0):
    ...
    if 100. * test_correct / test_total > max_acc_in:
        ...
        torch.save(model, f"{model_path}/iter_{iteration}_finetune_{epoch + 1}.pt")
    return acc
...
torch.save(model, f"{model_path}/iter_0_finetune_0.pt")

# open the file in the write mode
with open(f'{model_path}/{model_name}_iteration_max_accuracy.csv', 'a') as f:
...
while max_acc_in > 80:
    ...
    for fine_tune_epoch in range(finetune_epochs):
        ...
        test(model, fine_tune_epoch, iteration_in)

    # write a row to the csv file
    with open(f'{model_path}/{model_name}_iteration_max_accuracy.csv', 'a') as f:
        # create the csv writer
        writer = csv.writer(f)
        data = [iteration_in, max_acc_in]
        writer.writerow(data)

    iteration_in += 1
    print('Iteration : %d Test accuracy: %.2f %%' % (iteration_in, max_acc_in))
