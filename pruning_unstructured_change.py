...
parser.add_argument('--strategy', type=str, default="L1", help="L1, L2, unstructured, and random")
...
# prune: 0.05 ~ 0.9
# finetune: 0 ~ 200
model_path = f"{model_name}/{layer}/unstructured/prune_{prune_val}"
...
if model_name == 'mobilenetv1':
    ...
    if layer == 'all':
        ...
        model.bn1 = torch.nn.utils.prune.remove(model.bn1, name='weight')
        i = 0
        for m in model.modules():
            if isinstance(m, models.mobilenetv1.Block):
                # print("number: ", i)
                # print("Before: ", m.bn2.weight)
                prune.l1_unstructured(m.conv1, name='weight', amount=prune_val)
                m.conv1 = torch.nn.utils.prune.remove(m.conv1, name='weight')
                prune.l1_unstructured(m.bn1, name='weight', amount=prune_val)
                m.bn1 = torch.nn.utils.prune.remove(m.bn1, name='weight')
                prune.l1_unstructured(m.conv2, name='weight', amount=prune_val)
                m.conv2 = torch.nn.utils.prune.remove(m.conv2, name='weight')
                prune.l1_unstructured(m.bn2, name='weight', amount=prune_val)
                m.bn2 = torch.nn.utils.prune.remove(m.bn2, name='weight')
                # print("After: ", m.bn2.weight)
                i += 1
    elif layer == 'one':
        # i = 0
        for m in model.modules():
    ...
    if layer == 'all':
        ...
        for m in model.modules():
            if isinstance(m, models.mobilenetv2.Block):
                ...
                prune.l1_unstructured(m.bn2, name='weight', amount=prune_val)
                m.bn2 = torch.nn.utils.prune.remove(m.bn2, name='weight')
                prune.l1_unstructured(m.conv3, name='weight', amount=prune_val)
                m.conv3 = torch.nn.utils.prune.remove(m.conv3, name='weight')
                prune.l1_unstructured(m.bn3, name='weight', amount=prune_val)
                m.bn3 = torch.nn.utils.prune.remove(m.bn3, name='weight')
        prune.l1_unstructured(model.conv2, name='weight', amount=prune_val)
        model.conv2 = torch.nn.utils.prune.remove(model.conv2, name='weight')
        prune.l1_unstructured(model.bn2, name='weight', amount=prune_val)
        model.bn2 = torch.nn.utils.prune.remove(model.bn2, name='weight')
    ...
    if layer == 'all':
        ...
        prune.l1_unstructured(model.bn2, name='weight', amount=prune_val)
        model.bn2 = torch.nn.utils.prune.remove(model.bn2, name='weight')
        prune.l1_unstructured(model.conv3, name='weight', amount=prune_val)
        model.conv3 = torch.nn.utils.prune.remove(model.conv3, name='weight')
    if layer == 'one':
        for m in model.modules():
            if isinstance(m, models.mobilenetv3.Block):
                prune.l1_unstructured(m.conv1, name='weight', amount=prune_val)
    ...
    if layer == 'all':  # Conv2d
        ...
        prune.l1_unstructured(model.linear1, name='weight', amount=prune_val)
        model.linear1 = torch.nn.utils.prune.remove(model.linear1, name='weight')
        prune.l1_unstructured(model.linear2, name='weight', amount=prune_val)
        model.linear2 = torch.nn.utils.prune.remove(model.linear2, name='weight')
    elif layer == 'one':
        for m in model.modules():
            if isinstance(m, nn.Conv2d):
                prune.l1_unstructured(m, name='weight', amount=prune_val)
    ...
    if layer == 'all':
        ...
        for m in model.modules():
            if isinstance(m, models.efficientnet.Block):
                ...
                prune.l1_unstructured(m.bn2, name='weight', amount=prune_val)
                m.bn2 = torch.nn.utils.prune.remove(m.bn2, name='weight')
                prune.l1_unstructured(m.se_linear1, name='weight', amount=prune_val)
                m.se_linear1 = torch.nn.utils.prune.remove(m.se_linear1, name='weight')
                prune.l1_unstructured(m.se_linear2, name='weight', amount=prune_val)
                m.se_linear2 = torch.nn.utils.prune.remove(m.se_linear2, name='weight')
                prune.l1_unstructured(m.conv3, name='weight', amount=prune_val)
                m.conv3 = torch.nn.utils.prune.remove(m.conv3, name='weight')
                prune.l1_unstructured(m.bn3, name='weight', amount=prune_val)
                m.bn3 = torch.nn.utils.prune.remove(m.bn3, name='weight')
...
# for m in model.modules():
#     if isinstance(m, models.mobilenetv1.Block):
#         print(list(m.bn2.named_parameters()))
...
with open(f'{model_name}/{layer}/unstructured/no_finetuning_accuracy.csv', 'a') as f:
    writer = csv.writer(f)
    data = [prune_val, first_acc]
    writer.writerow(data)

# Number of parameter
with open(f'{model_name}/{layer}/unstructured/parameter_num.csv', 'a') as f1:
...
with open(f'{model_name}/{layer}/unstructured/finetuning_best_accuracy.csv', 'a') as f2:
